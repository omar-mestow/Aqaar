export const useGetSiteApi = () => {
  ...
  // Get All Data Lazy Mode
  const LazyGetAll = async (endpoint) => useLazyFetch(() => `${endpoint}`, {
    key: `${endpoint}`,
    baseURL: apiBase,
    headers: {
      'Authorization': `Bearer ${useMainToken().value}`
  ...
  const GetByIdtype = async (endpointWithoutId, id,type) => useFetch(`${endpointWithoutId}${id}/${type}`, {
    key: `${id}`,
    baseURL: apiBase,
    headers: {
      'Authorization': `Bearer ${useMainToken().value}`
